...
In this file are implemented many 2-dimensional packing algorithms (or strip packing algorithms). All algorithms are
taken from Jukka Jylang (2010) - A Thousand Ways to Pack the Bin: A Practical Approach to Two-Dimensional Rectangle
Bin Packing.
...
@functools.lru_cache(maxsize=cachesize)
def GBL (cases, layersize, splitting="shorteraxis"):
    """
                            * Guilliotine Bottom Left *
    ...
    In this particular case priority is given to the spaces closer to the bottom
    left corner.
    """
    X, Y = layersize
    F = [(0, 0, X, Y), ]  # (x, y, sizex, sizey) For each rectangle
    ...
    for case in cases:
        # Sort spaces prioritizing those close to the bottom left corner.
...
@functools.lru_cache(maxsize=cachesize)
def MRBAF (cases, layersize):
    """
                         * Maximal Rectangles Best Area Fit *
    ...
    F = [(0, 0, X, Y), ]  # (x, y, sizex, sizey) For each rectangle
    for case in cases:
        # Prioritize small spaces
        F.sort(key=lambda i: i[2]*i[3])
        for x, y, sizex, sizey in F:
            # Preliminar control on area
            if sizex * sizey < case.sizex * case.sizey:
                continue
